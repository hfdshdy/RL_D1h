...
from RL_D1h.tasks.manager_based.rl_d1h.velocity_env_cfg import (
    LocomotionVelocityFlatEnvCfg,
    CurriculumCfg,
)

from RL_D1h.assets.D1h import D1H_CFG  # isort: skip


@configclass
class D1HCurriculumCfg(CurriculumCfg):
...
class D1HRewardsCfg():
    ...
    undesired_contacts = RewTerm(
        ...
        weight=-0.5,
        params={
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=[ "base_link", ".*_thigh",".*_calf", ".*_hip"]),
            "threshold": 1.0,
        },
    )
...
@configclass
class D1HFlatEnvCfg(LocomotionVelocityFlatEnvCfg):

    rewards: D1HRewardsCfg = D1HRewardsCfg()
    # curriculum: D1HCurriculumCfg = D1HCurriculumCfg()
    ...
    def __post_init__(self):
       ...
       #! ************** scene & observations setup - 0 *********** !#
        ...
        self.events.physics_material.params["asset_cfg"].body_names = [".*_hip", ".*_thigh", ".*_calf", ".*_foot",".*base.*",]
