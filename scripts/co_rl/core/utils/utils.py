...
def store_code_state(logdir, repositories) -> list:
    ...
    for repository_file_path in repositories:
        ...
        try:
            ...
            diff_content = repo.git.diff(tree)
        except Exception as exc:
            print(f"Could not resolve HEAD for '{repo_name}': {exc}. Storing status only.")
            diff_content = "<diff unavailable: failed to resolve HEAD>"
        # write the diff file
        print(f"Storing git diff for '{repo_name}' in: {diff_file_name}")
        with open(diff_file_name, "x", encoding="utf-8") as f:
            try:
                status_content = repo.git.status()
            except Exception as exc:
                status_content = f"<status unavailable: {exc}>"
